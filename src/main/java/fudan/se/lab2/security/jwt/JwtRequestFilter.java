...
public class JwtRequestFilter extends OncePerRequestFilter {
    ...
    public void identity(String username,String jwtToken,HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws IOException, ServletException {
        //普通用户
        if (userRepository.findByUsername(username) != null  && SecurityContextHolder.getContext().getAuthentication() == null) {
            if(!jwtTokenUtil.isTokenExpired(jwtToken)){
                filterChain.doFilter(request, response);
            }
        }
        //管理员
        else if((request.getRequestURI().contains("admin") || request.getRequestURI().contains("passContact") || request.getRequestURI().contains("refuseContact")) && administratorRepository.findByName(username)!=null && SecurityContextHolder.getContext().getAuthentication() == null){
            if(!jwtTokenUtil.isTokenExpired(jwtToken)){
                filterChain.doFilter(request, response);
            }
        }
